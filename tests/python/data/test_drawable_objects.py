...
from python.data import (
    Point, Color, Line, Circle, Ellipse, Rectangle, 
    Polygon, Polyline, Arc, Text, Path, PathCommand,
    LineStyle, TextAlignment, VerticalAlignment, FillStyle
)
...
class TestLine:
    ...
    def test_line_bounding_box(self):
        line = Line(
            start_point=Point(x=20, y=30),
            end_point=Point(x=10, y=50)
        )
    ...
    def test_text_validation(self):
        with pytest.raises(ValidationError):
            Text(
                position=Point(x=0, y=0),
                content="Test",
                font_size=0  # font_size must be > 0
            )
    ...
    def test_opacity_validation(self):
        with pytest.raises(ValidationError):
            Circle(
                center=Point(x=0, y=0),
                radius=10,
                opacity=1.5  # opacity must be <= 1.0
            )
        ...
        with pytest.raises(ValidationError):
            Circle(
                center=Point(x=0, y=0),
                radius=10,
                opacity=-0.1  # opacity must be >= 0.0
            )