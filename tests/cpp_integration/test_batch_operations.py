...
def test_batch_operations():
    ...
    drawing_cpp.BatchOperations.rotate_objects(
        storage, grid_ids, 
        math.radians(45), center
    )
...
def benchmark_batch_vs_individual():
    ...
    operations = [
        ...
        ("Calculate bbox", lambda s, ids: drawing_cpp.BatchOperations.calculate_bounding_box(s, ids)),
    ]
    ...
    for op_name, op_func in operations:
        ...
        if stats.objects_processed > 0:
            print(f"  {op_name}: {elapsed:.2f} ms ({stats.objects_per_second/1e6:.1f}M objects/sec)")
