...
def compare_colors(py_color, cpp_color, tolerance=0):
    ...
    return (abs(py_color.r - cpp_color.r) <= tolerance and
            abs(py_color.g - cpp_color.g) <= tolerance and
            abs(py_color.b - cpp_color.b) <= tolerance and
            abs(py_color.a - cpp_color.a) <= tolerance)

def compare_points(py_point, cpp_point, tolerance=0.001):
    """Compare Python and C++ points"""
    return (abs(py_point.x - cpp_point.x) <= tolerance and
            abs(py_point.y - cpp_point.y) <= tolerance)

def compare_bounding_boxes(py_bbox, cpp_bbox, tolerance=0.001):
    """Compare Python and C++ bounding boxes"""
    return (abs(py_bbox.min_x - cpp_bbox.min_x) <= tolerance and
            abs(py_bbox.min_y - cpp_bbox.min_y) <= tolerance and
            abs(py_bbox.max_x - cpp_bbox.max_x) <= tolerance and
            abs(py_bbox.max_y - cpp_bbox.max_y) <= tolerance)
...
class CompatibilityTester:
    ...
    def test_basic_types(self):
        ...
        self.assert_close(py_color.a, cpp_color.a / 255.0, 0.01, "Color alpha component (normalized)")
    ...
    def test_line_styles(self):
        ...
        py_line_solid = PyLine(start_point=PyPoint(x=0, y=0), end_point=PyPoint(x=100, y=100), line_style=PyLineStyle.SOLID)
        py_line_dashed = PyLine(start_point=PyPoint(x=0, y=0), end_point=PyPoint(x=100, y=100), line_style=PyLineStyle.DASHED)
    ...
    def test_drawable_objects(self):
        ...
        py_rect = PyRectangle(top_left=PyPoint(x=200, y=200), width=150, height=100, corner_radius=10)
        py_line = PyLine(start_point=PyPoint(x=0, y=0), end_point=PyPoint(x=50, y=50), line_style=PyLineStyle.DOTTED)
        py_ellipse = PyEllipse(center=PyPoint(x=300, y=300), rx=60, ry=40, rotation=45)
        py_polygon = PyPolygon(points=[PyPoint(x=0, y=0), PyPoint(x=100, y=0), PyPoint(x=50, y=100)], closed=True)
        py_polyline = PyPolyline(points=[PyPoint(x=0, y=0), PyPoint(x=50, y=50), PyPoint(x=100, y=0)], line_style=PyLineStyle.DASHED)
        py_arc = PyArc(center=PyPoint(x=400, y=400), radius=30, start_angle=0, end_angle=90)
        py_text = PyText(position=PyPoint(x=500, y=500), content="Test Text", font_size=16, font_family="Arial")
        py_path = PyPath(commands=[
        ...
        cpp_polygon = cpp_drawing.add_polygon([cpp.Point(0, 0), cpp.Point(100, 0), cpp.Point(50, 100)], True)
        cpp_polyline = cpp_drawing.add_polyline([cpp.Point(0, 0), cpp.Point(50, 50), cpp.Point(100, 0)], cpp.LineStyle.Dashed)
    ...
    def test_advanced_features(self):
        ...
        py_circle = PyCircle(center=PyPoint(x=100, y=100), radius=50, metadata={"type": "logo", "layer": "graphics"})
    ...
    def test_performance_comparison(self):
        ...
        self.assert_equal(True, cpp_memory / cpp_drawing.total_objects() < 100, "C++ memory < 100 bytes per object")
    ...
    def test_serialization_compatibility(self):
        ...
        try:
            ...
            self.assert_equal(cpp_drawing.total_objects(), loaded_drawing.total_objects(), "Binary serialization object count")
            self.assert_close(cpp_drawing.get_width(), loaded_drawing.get_width(), message="Binary serialization width")
            self.assert_close(cpp_drawing.get_height(), loaded_drawing.get_height(), message="Binary serialization height")
        ...
        try:
            ...
            with open("test_compatibility.json", 'r') as f:
                ...
                self.assert_equal(True, "objects" in json_data or "width" in json_data, "JSON structure")
    ...
    def run_all_tests(self):
        ...
        print(f"🎯 Compatibility: {'EXCELLENT' if success_rate >= 95 else 'GOOD' if success_rate >= 85 else 'NEEDS_WORK'}")
